...
import { useGameData, PLOT_COST_SAS, SHIELD_COST_SAS, MAX_PLOTS_PER_PLAYER, BASE_PLOTS_COUNT } from '../stores/gameDataStore'
...
export default function Farm() {
  ...
  useEffect(() => {
    if (connected && !isInitialized) {
      initializePlayer(10000, 0) // Start with 10k USDT and 0 SAS for testing
    }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      tickYield({
        ...
        sasApyPercent: settings.apyPercent / 2, // SAS APY is half of USDT APY
  ...
  const handleBuyShield = useCallback(() => {
    const success = buyShield(1) // Buy 1 shield
    if (success) {
    ...
    <div className="min-h-screen pb-24 px-4 pt-4">
      ...
      <div className="space-y-3 mb-6">
        {pendingYield > 0 && (
          <motion.button
            ...
            className="w-full py-4 bg-gradient-to-r from-green-500 to-emerald-600 rounded-xl font-bold text-lg flex items-center justify-center gap-2 text-white text-outline-green"
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
          >
            <SparklesIcon className="w-6 h-6" />
            <span className="truncate">Claim {formatBalance(pendingYield)} USDT Yield</span>
          </motion.button>
        )}
        {pendingSasRewards > 0 && (
          <motion.button
            onClick={handleClaimSasRewards}
            className="w-full py-4 bg-gradient-to-r from-purple-500 to-pink-600 rounded-xl font-bold text-lg flex items-center justify-center gap-2 text-white"
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
          >
            <SparklesIcon className="w-6 h-6" />
            <span className="truncate">Claim {formatBalance(pendingSasRewards)} SAS Rewards</span>
          </motion.button>
        )}
      </div>
      ...
      <div className="mt-8 p-4 bg-slate-800/60 rounded-xl border border-slate-600">
        ...
        <ul className="text-sm text-white text-outline-dark space-y-2">
          <li>• You start with {BASE_PLOTS_COUNT} plots, can buy up to {MAX_PLOTS_PER_PLAYER} total</li>
          <li>• Stake USDT to earn both USDT yield and SAS rewards</li>
          <li>• Only YOU can see which plots have tokens</li>
          <li>• Raiders must guess the correct plot to steal</li>
          <li>• 🛡️ <span className="text-blue-400">Shields protect ALL plots</span> from one raid attempt</li>
        ...
        {depositModalPlotId !== null && (
          ...
          >
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-sm text-white/80 mb-1.5 block">Amount (USDT)</label>
                  <div className="relative">
                    ...
                    />
                    <button
                      onClick={() => setDepositAmount(usdtBalance.toString())}
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  <button
                    ...
                    disabled={!depositAmount || parseFloat(depositAmount) <= 0 || parseFloat(depositAmount) > usdtBalance}
