...
export function simulateFaucetForDemo(currentBalance: string): string {
  // Handle fractional numbers by truncating to integer first
  const currentNum = Math.floor(parseFloat(currentBalance || '0'));
  const faucetAmount = 10000; // Add 10000 tokens
  return (currentNum + faucetAmount).toString();
}
