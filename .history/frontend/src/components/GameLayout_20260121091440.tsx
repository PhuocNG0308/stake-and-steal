...
// Navigatfarm', icon: BanknotesIcon, label: 'Stake' },
  { to: '/raid', icon: FireIcon, label: 'Steal' },
  { to: '/', icon: HomeIcon, label: 'Home' },
  { to: '/stats', icon: ChartBarIcon, label: 'Stats' },
  { to: '/settings', icon: Cog6ToothIcon, label: 'Setting },
    ...
    <div className="min-h-screen relative overflow-hidden font-game">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50">
        ...
        <div className="h-20 w-full bg-[#0D47A1] border-t-[3px] border-[#FF9800] flex items-stretch shadow-[0_-4px_10px_rgba(0,0,0,0.5)] relative">
          ...
          {/* Tab 1: Stake (Farm) */}
          <NavLink to="/farm" className="flex-1 group relative outline-none">
            {({ isActive }) => (
              <div className="w-full h-full flex items-center justify-center relative">
                {/* Separator */}
                <div className="absolute inset-y-4 right-0 w-px bg-black/20" />
                
                <div className={`flex flex-col items-center justify-center p-2 rounded-xl transition-all ${isActive ? 'bg-[#2196F3]/30' : 'group-hover:bg-white/5'}`}>
                  <SparklesIcon className={`w-6 h-6 ${isActive ? 'text-yellow-400' : 'text-white/60'}`} />
                </div>
              </div>
            )}
          </NavLink>
          ...
          {/* Tab 2: Steal (Raid) */}
          <NavLink to="/raid" className="flex-1 group relative outline-none">
            {({ isActive }) => (
              <div className="w-full h-full flex items-center justify-center relative">
                {/* Separator - Hidden on right side next to center button? keep it for visual balance */}
                <div className="hidden sm:block absolute inset-y-4 right-0 w-px bg-black/20" />
                
                <div className={`flex flex-col items-center justify-center p-2 rounded-xl transition-all ${isActive ? 'bg-[#2196F3]/30' : 'group-hover:bg-white/5'}`}>
                  <BoltIcon className={`w-6 h-6 ${isActive ? 'text-yellow-400' : 'text-white/60'}`} />
                </div>
              </div>
            )}
          </NavLink>

          {/* Tab 3: HOME (Breaking the Grid) */}
          <div className="flex-1 relative flex justify-center z-10 pointer-events-none">
            {/* The actual clickable area needs pointer-events-auto */}
            <NavLink to="/" className="absolute -top-8 pointer-events-auto outline-none">
              {({ isActive }) => (
                <motion.div 
                  className={`w-20 h-20 rounded-full border-[4px] border-[#FF9800] flex flex-col items-center justify-center shadow-2xl transition-all
                    ${isActive 
                      ? 'bg-gradient-to-br from-[#42A5F5] to-[#1565C0]' 
                      : 'bg-[#1565C0] hover:bg-[#1976D2]'
                    }`}
                  whileTap={showAnimations ? buttonTap : {}}
                  style={{ boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.4), 0 2px 4px -1px rgba(0, 0, 0, 0.2)' }}
                >
                  <HomeIcon className={`w-9 h-9 ${isActive ? 'text-white' : 'text-slate-300'}`} />
                  {isActive && (
                    <span 
                      className="text-[11px] font-black text-white leading-none mt-0.5"
                      style={{ 
          ...
          {/* Tab 4: Stats */}
          <NavLink to="/stats" className="flex-1 group relative outline-none">
            {({ isActive }) => (
              <div className="w-full h-full flex items-center justify-center relative">
                <div className="hidden sm:block absolute inset-y-4 left-0 w-px bg-black/20" />
                
                <div className={`flex flex-col items-center justify-center p-2 rounded-xl transition-all ${isActive ? 'bg-[#2196F3]/30' : 'group-hover:bg-white/5'}`}>
                  <ChartBarIcon className={`w-6 h-6 ${isActive ? 'text-yellow-400' : 'text-white/60'}`} />
                </div>
              </div>
            )}
          </NavLink>
           ...
           {/* Tab 5: Settings */}
          <NavLink to="/settings" className="flex-1 group relative outline-none">
            {({ isActive }) => (
              <div className="w-full h-full flex items-center justify-center relative">
                <div className="absolute inset-y-4 left-0 w-px bg-black/20" />
                
                <div className={`flex flex-col items-center justify-center p-2 rounded-xl transition-all ${isActive ? 'bg-[#2196F3]/30' : 'group-hover:bg-white/5'}`}>
                  <Cog6ToothIcon className={`w-6 h-6 ${isActive ? 'text-yellow-400' : 'text-white/60'}`} />
                </div>
              </div>
            )}
          </NavLink>
