  ...
  persist(
    (set, get) => ({
      ...
      executeRaid: (victimId: string, plotIndex: number) => {
        const { testVictims, usdtBalance, stats, raidHistory } = get()
        ...
        if (hasTokens && plotBalance > 0) {
          ...
          return { success: true, amount: stealAmount, blockedByShield: false }
        } else {
          // Failed - wrong plot
          const historyEntry: RaidHistoryEntry = {
            id: `raid-${Date.now()}`,
      ...
      createTestVictim: () => {
        ...
        const plotConfig = Array(BASE_PLOTS_COUNT).fill(false)
        const plotBalances = Array(BASE_PLOTS_COUNT).fill(0)
        
        // Randomly select which plots have tokens
        const indices = Array.from({ length: BASE_PLOTS_COUNT }, (_, i) => i)
        ...
        // Randomly give some victims shields (0-3)
        const shields = Math.random() > 0.5 ? Math.floor(Math.random() * 3) : 0
      ...
      registerOnNetwork: () => {
        const { playerFarm, currentWalletId, networkPlayers, inventory } = get()
      ...
      addUsdtBalance: (amount: number) => {
        const { usdtBalance } = get()
        set({ usdtBalance: usdtBalance + amount })
      },

      addSasBalance: (amount: number) => {
        const { sasBalance } = get()
        set({ sasBalance: sasBalance + amount })
      },

      removeUsdtBalance: (amount: number) => {
        const { usdtBalance } = get()
        if (usdtBalance < amount) return false
        set({ usdtBalance: usdtBalance - amount })
        return true
      },

      removeSasBalance: (amount: number) => {
        const { sasBalance } = get()
        if (sasBalance < amount) return false
        set({ sasBalance: sasBalance - amount })
        return true
      },

      // Legacy balance methods (backward compatibility - use USDT)
      ...
      calculatePendingYield: (settings) => {
        ...
        // Calculate USDT yield for each plot individually based on its deposit time
      ...
      calculatePendingSasRewards: (settings) => {
        const { playerFarm } = get()
        if (!playerFarm || playerFarm.totalStaked === 0) return 0
      ...
      tickYield: (settings) => {
        ...
        // Calculate yields based on actual deposit times
        const newYield = get().calculatePendingYield(settings)
        const newSasRewards = get().calculatePendingSasRewards(settings)
        
        // Only update if changed significantly
        const yieldChanged = Math.abs(newYield - playerFarm.pendingYield) > 0.0001
        const sasChanged = Math.abs(newSasRewards - playerFarm.pendingSasRewards) > 0.0001
        
        if (yieldChanged || sasChanged) {
          set({
            playerFarm: {
              ...playerFarm,
              pendingYield: newYield,
              pendingSasRewards: newSasRewards,
            },
            lastTickTime: Date.now(),
          })
        }
...
export function useGameData() {
  ...
  const totalPlots = store.inventory.totalPlots
  const shields = store.inventory.shields
